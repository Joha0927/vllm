...
from benchmarks.glm52_layer_profiling.config import BenchmarkConfig, manifest
...
def engine_kwargs(config: BenchmarkConfig) -> dict[str, Any]:
    kwargs: dict[str, Any] = {
        ...
        "enforce_eager": True,
        "hf_overrides": {"num_hidden_layers": config.num_layers},
...
def _model_parameter_evidence(model: Any) -> dict[str, Any]:
    count = 0
    parameter_bytes = 0
    dtype_bytes: defaultdict[str, int] = defaultdict(int)
    for parameter in model.parameters():
        elements = parameter.numel()
        size = elements * parameter.element_size()
        count += elements
    ...
    return {
        ...
        "persistent_tensor_bytes": parameter_bytes + buffer_bytes,
    }
...
def run(config: BenchmarkConfig) -> None:
    ...
    parameter_evidence = llm.apply_model(_model_parameter_evidence)
    if not parameter_evidence or any(
        item["model_class"] != "DeepseekV32ForCausalLM"
        or item["parameter_count"] <= 0
        or item["parameter_bytes"] <= 0
        or item["persistent_tensor_bytes"] < item["parameter_bytes"]
        for item in parameter_evidence
    ):
        raise RuntimeError("production workers returned invalid parameter evidence")
    ...
    runtime_evidence = {
        **rank_evidence,
        ...
        "worker_parameter_evidence": parameter_evidence,
    }
    print(
    ...
    barrier()
    if config.profile == "torch":
        llm.start_profile("glm52_first_seven_layers")
        barrier()
    try:
